...
cl = yuda.LINE()
#cl.login(qr=True)
cl.login(token='isi token mu')#ranita
cl.loginResult()#
...
Bots=[mid]
owner=["uecc57cb55f480ee2a45d81434a9b864d",mid]
admin=["uecc57cb55f480ee2a45d81434a9b864d",mid]
baby=["uecc57cb55f480ee2a45d81434a9b864d"]#chery/barby/ranita
creator=["uecc57cb55f480ee2a45d81434a9b864d"]
owner=["uecc57cb55f480ee2a45d81434a9b864d"]
wait = {
    'contact':False,
    'autoJoin':True,
...
#						cl.comment(hasil['result']['posts'][zx]['userInfo']['mid'],hasil['result']['posts'][zx]['postInfo']['postId'],"Auto Like By Yudha!!\nID LINE : line://ti/p/~iniyud\nline.me/ti/p/~@ikj9968f")
#						print "DiLike"
#					except:
#							pass
#				else:
    ...
    try:
        ...
        if op.type == 13:
            if wait["auto"] == True:
                ...
                cl.sendText(op.param1, "Terima Kasih Telah Invite 👉😊☆º°˚˚☆✰ ΥυδHΔ βΟτ ✰☆º°˚˚☆（＾ω＾）\n\nby Yudha👈 »»» http://line.me/ti/p/~iniyud «««\n»»» http://line.me/ti/p/~@ikj9968f «««\nSilahkan ketik [Help],dan gunakan dgn bijak ya kak")
        if op.type == 5:
        ...
        if op.type == 26:
            ...
            if "@"+cl.getProfile().displayName in msg.text:
                if wait["tag"] == True:
                    ...
                    jawab = ("Kenapa Tag Si "+cl.getProfile().displayName+"Kangen yah..!!!\nPC aja langsung biar anu hihi..!!\n[autoRespon]by=>SelfBot~Yudha","Nah ngetag lagi si "+cl.getProfile().displayName+" mending ajak mojok aja dari pada ngetag mulu.. wkwk...!!!\n[autoRespon]by=>SelfBot~Yudha")
...
#--------------------------------------------------
            elif "Yud1 kick " in msg.text:
				if msg.from_ in admin or owner:
					midd = msg.text.replace("Kr1 kick ","")
					ki.kickoutFromGroup(msg.to,[midd])
#--------------------------------------------------
            elif "Yud2 kick " in msg.text:
				if msg.from_ in admin or owner:
					midd = msg.text.replace("Kr2 kick ","")
					kk.kickoutFromGroup(msg.to,[midd])
#--------------------------------------------------
            elif "Yud3 kick " in msg.text:
				if msg.from_ in admin or owner:
...
#--------------------------------------------------
                ...
                if msg.from_ in admin or owner:
                    ...
                    cl.sendMessage(msg)
#--------------------------------------------------
            elif msg.text in ["Yud1"]:
				if msg.from_ in admin or owner:
					msg.contentType = 13
					msg.contentMetadata = {'mid': Amid}
					ki.sendMessage(msg)
#--------------------------------------------------
            elif msg.text in ["Yud2"]:
				if msg.from_ in admin or owner:
					msg.contentType = 13
					msg.contentMetadata = {'mid': Bmid}
					kk.sendMessage(msg)
#--------------------------------------------------
            elif msg.text in ["Cancel","Yud cancel"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
...
#--------------------------------------------------
            elif msg.text in ["Yud1 ourl","Yud1 link on"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.preventJoinByTicket = False
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------
            elif msg.text in ["Yud2 ourl","Yud2 link on"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.preventJoinByTicket = False
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------
            elif msg.text in ["Yud3 ourl","Yud3 link on"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
				...
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------
            elif msg.text in ["Yud1 curl","Yud1 link off"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.preventJoinByTicket = True
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------
            elif msg.text in ["Yud2 curl","Yud2 link off"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						X = cl.getGroup(msg.to)
						X.preventJoinByTicket = True
						...
						if wait["lang"] == "JP":
							...
							cl.sendText(msg.to,"Not for use less than group")
#--------------------------------------------------
            elif msg.text in ["Yud3 curl","Yud3 link off"]:
				if msg.from_ in admin or owner:
					if msg.toType == 2:
						X = kc.getGroup(msg.to)
						X.preventJoinByTicket = True
...
#--------------------------------------------------
				...
				if msg.from_ in admin or owner:
					cl.sendText(msg.to,mid)
#--------------------------------------------------
            elif "Yud1 mid" == msg.text:
				if msg.from_ in admin or owner:
					cl.sendText(msg.to,Amid)
#--------------------------------------------------
            elif "Yud2 mid" == msg.text:
				if msg.from_ in admin or owner:
					cl.sendText(msg.to,Bmid)
#--------------------------------------------------
            elif "Yud3 mid" == msg.text:
				if msg.from_ in admin or owner:
					kc.sendText(msg.to,Cmid)
#--------------------------------------------------
...
#-----------------------------------------------
				...
				if msg.from_ in admin or owner or mid:
					cl.sendText(msg.to,"Masuk")
#-----------------------------------------------
            elif msg.text in ["Yud mode on","yud mode on"]:
                if msg.from_ in admin or owner:
                    if wait["QrProtect"] == True:
                        if wait["lang"] == "JP":
                    ...
                    if wait["Protectguest"] == True:
                        ...
                        if wait["lang"] == "JP":
                            ...
                            cl.sendText(msg.to,"done")
#=================================================
            elif msg.text in ["Yud mode off","yud mode off"]:
                if msg.from_ in admin or owner:
                    ...
                    if wait["Lv"] == False:
                        ...
                        wait["Lv"] = False
                        if wait["lang"] == "JP":
                            cl.sendText(msg.to,"Nyapa yg left already oғғ")
...
#-----------------------------------------------
					...
					bctxt = msg.text.replace("Yud say ","")
...
#-----------------------------------------------
                ...
                jawab = ("10%","20%","30%","40%","50%","60%","70%","80%","90%","99%","100%")
                jawaban = random.choice(jawab)
                cl.sendText(msg.to,jawaban)
#-----------------------------------------------
...
#-----------------------------------------------
              ...
              if msg.from_ in admin or owner:
                if admin == []:
                    ...
                    cl.sendText(msg.to,"Sabar Dikit Boss.....")
                    mc = ""
                    for mi_d in admin:
                ...
                if msg.from_ in admin or owner:
                  ...
                  for g in gs.members:
                      if _nametarget == g.displayName:
                          ...
                          pass
#--------------------------
            elif msg.text in ["Yud kemari"]: # Keluar Dari Semua Group Yang Di dalem nya  ada bot(Kalo Bot Kalian Nyangkut di Group lain :D)
              if msg.from_ in admin or owner:
                gid = cl.getGroupIdsJoined()
                for i in gid:
                  cl.leaveGroup(i)
...
#------------------------------------------------------------------
            ...
            elif msg.text in ["yud glist"]: #Melihat List Group
                if msg.from_ in owner:
                    gids = cl.getGroupIdsJoined()
                    h = ""
                    for i in gids:
                ...
                if msg.from_ in owner:
                    if msg.toType == 2:
                        ...
                        try:
                            ...
                            pass
            elif "Yud megs " in msg.text:
                if msg.from_ in owner:
                    gName = msg.text.replace("yud megs ","")
            ...
            elif msg.text in ["Yud muach"]:
                if msg.from_ in owner:
                    msg.contentType = 13
                    msg.contentMetadata = {'mid': "ua7fb5762d5066629323d113e1266e8ca',"}
                    cl.sendMessage(msg)
            elif msg.text in ["yud","Yud"]:
                if msg.from_ in owner in mid:
                    cl.sendText(msg.to,"Yudha bot masih aktif Boss...!!!")
